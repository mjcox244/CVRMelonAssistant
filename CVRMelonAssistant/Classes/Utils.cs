    ...
    public class Utils
    {
        ...
        public static string GetInstallDir()
        {
            ...
            if (!string.IsNullOrEmpty(InstallDir)
                ...
                && Directory.Exists(Path.Combine(InstallDir, "ChilloutVR_Data", "Plugins"))
                && File.Exists(Path.Combine(InstallDir, "ChilloutVR.exe")))
            {
                return InstallDir;
            }
        ...
        {
            App.ChilloutInstallDirectory = directory;
            App.ChilloutInstallType = store;
        ...
        public static string GetSteamDir()
        {
            ...
            using (StreamReader reader = new(@vdf))
            {
                ...
                {
                    Match match = libFolderRegex.Match(line);
                    if (match.Success)
                    {
                ...
                if (File.Exists(Path.Combine(@path, @"appmanifest_" + Constants.ChilloutVRAppId + ".acf")))
                {
                    using (StreamReader reader = new(Path.Combine(@path, @"appmanifest_" + Constants.ChilloutVRAppId + ".acf")))
                    {
                        ...
                        while ((line = reader.ReadLine()) != null)
                        {
                            ...
                            if (match.Success)
                            {
                                if (File.Exists(Path.Combine(@path, @"common", match.Groups[1].Value, "ChilloutVR.exe")))
                                {
        ...
        public static string GetOculusDir()
        {
            ...
            {
                if (File.Exists(Path.Combine(OculusInstall, "Software", "ChilloutVR-ChilloutVR", "ChilloutVR.exe")))
                {
                    return SetDir(Path.Combine(OculusInstall, "Software", "ChilloutVR-ChilloutVR"), "Oculus");
                }
            }
                ...
                using (ManagementObjectSearcher searcher = new(wqlQuery))
                {
                    ...
                    foreach (string libraryKeyName in librariesKey.GetSubKeyNames())
                    {
                        using (RegistryKey libraryKey = librariesKey.OpenSubKey(libraryKeyName))
                        {
                            ...
                            {
                                string finalPath = Path.Combine(GUIDLetter, libraryPath.Substring(49), @"Software\ChilloutVR-ChilloutVR");
                                if (File.Exists(Path.Combine(finalPath, "ChilloutVR.exe")))
                                {
        ...
        public static string GetManualDir()
        {
            ...
            if (dialog.ShowDialog() == true)
            {
                ...
                if (File.Exists(Path.Combine(path, "ChilloutVR.exe")))
                {
                    string store;
                    if (File.Exists(Path.Combine(path, "ChilloutVR_Data", "Plugins", "steam_api64.dll")))
                    {
                        store = "Steam";
                    }
                    else
